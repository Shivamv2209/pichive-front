  ...
  const handleFindPhotos = async () => {
    try {
      ...
      const result = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/search/${event_code}`,
        formData,
      );
            ...
            <div className="w-full max-w-[450px] bg-[#1e1e1e] p-5 md:p-6 mt-6 rounded-[32px]">
                ...
                {capturedImage ? (
                  <div className="flex flex-col sm:flex-row items-center gap-4">
                    <button
                      ...
                      className="w-full sm:w-auto bg-white text-black px-6 py-3 rounded-2xl"
                    >
                      Find My Photos
                    </button>
                    <button
              ...
              <div className="mt-4 p-4 rounded-xl bg-green-50 border border-green-200">
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        Your selfie is used only to find your photos and is never permanently stored.
                      </p>
                    </div>
            </div>
          </div>
